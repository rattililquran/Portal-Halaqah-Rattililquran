(function() {
  ...
  async function loadMutabaahGuru() {
    ...
    try {
      var r = await window.HQ.GuruAPI.getMutabaahDaurahGuru(id_periode);
      _mtbData = r.data;
      _expandedHq = {};
      _populateMtbHeatmapFilter(_mtbData.halaqahList);
      renderMutabaahGuru(_mtbData);
    } catch(e) { toast('Gagal memuat Mutaba\'ah: ' + e.message, 'err'); }
    finally { hideLoad(); }
  }

  function renderMutabaahGuru(d) {
    renderMeHeader(d);
  ...
  function renderMeHeader(d) {
    var el = document.getElementById('mtbGuruHeader');
    if (!el) return;
    var p = d.periode;
    ...
    el.innerHTML =
      ...
      + '<div>'
      +   '<div style="font-size:20px;font-weight:900;color:var(--text);margin-bottom:4px">\ud83d\udccb Mutaba\'ah Daurah \u2014 ' + esc(p.nama_periode) + '</div>'
      ...
      +   '<select class="fc" id="mtbGuruPeriodeSel" onchange="loadMutabaahGuru()" style="max-width:220px;font-size:12px"><option value="P-DAURAH-JULI-2026">Daurah Al-Fatihah Jul 2026</option></select>'
      +   '<button class="btn btn-ghost btn-sm" onclick="loadMutabaahGuru()" style="display:flex;align-items:center;gap:6px"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg>Refresh</button>'
      +   '<button class="btn btn-outline btn-sm" onclick="mtbGuruCsvExport()" style="display:flex;align-items:center;gap:6px">\uD83D\uDCE5 Export CSV</button>'
      + '</div>'
      + '</div>'
  ...
  function renderMeSummaryCards(s, hariKe, statusDaurah) {
    var el = document.getElementById('mtbGuruSummaryCards');
    if (!el) return;
  ...
  function renderMeHalaqahTable(halaqahList, indikator) {
    var el = document.getElementById('mtbGuruTblBody');
    if (!el) return;
    ...
    el.innerHTML = halaqahList.map(function(hq) {
      ...
      var row = '<tr style="cursor:pointer" onclick="mtbGuruTglDetail(\'' + esc(hq.id_halaqah) + '\')">'
        + '<td><span style="font-size:11px;margin-right:6px;display:inline-block;transition:transform .2s" id="mtbGuruArr_' + esc(hq.id_halaqah) + '">\u25B6</span><strong>' + esc(hq.nama_halaqah) + '</strong></td>'
  ...
  function mtbGuruTglDetail(id_halaqah) {
    ...
    var row = document.getElementById('mtbGuruDetail_' + id_halaqah);
    var arr = document.getElementById('mtbGuruArr_' + id_halaqah);
  ...
  function renderMeHeatmap(halaqahList, indikator) {
    var wrap = document.getElementById('mtbGuruHeatmapWrap');
    if (!wrap) return;
    ...
    var filterHq = (document.getElementById('mtbGuruHeatmapFilter') || {}).value || '';
    var muridFlat = [];
  ...
  function _populateMtbHeatmapFilter(halaqahList) {
    var sel = document.getElementById('mtbGuruHeatmapFilter');
    if (!sel) return;
    var prev = sel.value;
  ...
  function renderMeInsight(ranking, alerts) {
    ...
    var alertEl = document.getElementById('mtbGuruAlertList');
    if (alertEl) {
      if (!alerts.length) { alertEl.innerHTML = '<div style="text-align:center;padding:20px;color:var(--text-3);font-size:12.5px">\u2705 Semua murid dalam kondisi baik</div>'; }
        ...
        var waBtn = m.no_hp
          ? '<button class="btn btn-outline btn-sm" style="border-color:#16a34a;color:#16a34a;margin-top:6px;font-size:11px;padding:2px 8px" onclick="openWAGuruAlert(\'' + escJs(m.nama_murid) + '\', \'' + escJs(m.no_hp) + '\', \'' + escJs(m.nama_halaqah) + '\', \'' + escJs(m.nama_guru) + '\', ' + m.pctHadir + ', ' + m.pahamCount + ', ' + m.tajwidBelum + ')" title="Hubungi via WhatsApp">💬 WA Guru</button>'
